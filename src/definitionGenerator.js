...
class DefinitionGenerator {
  ...
  async createOperationObject(method, documentation, name = uuid()) {
    ...
    if (documentation.requestBody)
      obj.requestBody = await this.createRequestBody(documentation).catch(
        (err) => {
          throw err;
        }
      );
    ...
    if (
      Object.keys(documentation).includes("x-amazon-apigateway-integration")
    ) {
      obj["x-amazon-apigateway-integration"] =
        documentation["x-amazon-apigateway-integration"];
    }
  ...
  async createResponseHeaders(headers) {
    ...
    for (const header of Object.keys(headers)) {
      ...
      if (headers[header].schema) {
        const schemaRef = await this.schemaCreator(
  ...
  async createMediaTypeObject(models, type) {
    const mediaTypeObj = {};
    for (const mediaTypeDocumentation of this.serverless.service.custom
  ...
  async dereferenceSchema(schema) {
    let originalSchema = await $RefParser
    ...
    let deReferencedSchema = await $RefParser
      .dereference(originalSchema, this.refParserOptions)
      .catch((err) => {
        console.error(err);
        throw err;
      });
    ...
    if (deReferencedSchema?.$ref === "#") {
      ...
      deReferencedSchema = await this.dereferenceSchema(
        deReferencedSchema
      ).catch((err) => {
        throw err;
      });
  ...
  async schemaCreator(schema, name) {
    ...
    const dereferencedSchema = await this.dereferenceSchema(schema).catch(
      (err) => {
        console.error(err);
        throw err;
      }
    ...
    for (const convertedSchemaName of Object.keys(convertedSchemas.schemas)) {
      ...
      if (this.existsInComponents(convertedSchemaName)) {
        if (
          this.isTheSameSchema(convertedSchema, convertedSchemaName) === false
        ) {
          if (convertedSchemaName === schemaName) {
            finalName = `${schemaName}-${uuid()}`;
            this.addToComponents(
              this.componentTypes.schemas,
              convertedSchema,
              finalName
            );
          } else
